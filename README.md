...
Η Ενέργεια υπολογίζεται αν αθροιστούν οι επιμέρους στατικές καταναλώσεις (Subthreshold Leakage, Gate Leakage) με την Runtime Dynamic για τον Core και την L2.
 ...
 - Energy-delay Product (EDP) = PDP * t [2]
...
## Σχετικά με την ακρίβεια των εξομοιώσεων
Η ανάλυσή για διάφορους λόγους αποκλίνει από την πραγματικότητα.
Το McPat από μόνο του μοντελοποιεί τους επεξεργαστές σε υψηλό επίπεδο μακριά το φυσικό, χρησιμοποιώντας απλά xml αρχεία για να πάρει πληροφορίες από τα χαρακτηριστικά του επεξεργαστή που από το μοντέλο τοο gem5 αλλά και τα εσωτερικά χαρα κτηριστικά από τις επιμέρους εκτελέσεις των benchmark. Η συνεκτίμηση της κατανάλωσης με βάση τα stats.txt δηλαδή θα έχει θετικό αντίκτυπο στην ακρίβεια αλλά και πάλι λείπει η εξομοίωση σε φυσικό επίπεδο τα τρανζίστορ.


## Σκέψεις
Η τρίτη εργασία έριξε φως στα θέματα κατανάλωσης ενέργειας 
των επεργαστών, πώς επηρεάζεται αυτή από από παραμέτρους του επεξεργαστή με τις οποίες είχαμε ήδη εξοικειωθεί στα προηγούμενα εργαστήρια. Ασκώντας κριτική στο εργαστήριο, θα έλεγα ότι αν και είχε αρκετή δουλειά, ένα μέρος μόνο αυτής είχε άμεση επαφή με το αντικείμενο και αρκετή ήταν διαδικαστική. Επιπλέον, αν και το μάθημα πραγματεύεται και άλλες αρχιτεκτονικές όπως VLIW, vector, και GPU, τα εργαστήρια είχαν να κανουν μόνο με scalar επεξεργατές. Ακόμα όμως κι έτσι, ασχοληθήκαμε μόνο με ένα μοντέλο στον gem5, τον MinorCPU ο οποίος είναι in-oder και άρα του λείπουν οι βελτιστοποιήσεις του out-of-order execution.

[1] https://technobyte.org/cmos-inverter-power-energy-consumption/

[2] https://www.eecs.umich.edu/courses/eecs470/OLD/w14/lectures/470L14W14.pdf
